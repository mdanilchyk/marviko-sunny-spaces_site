...
Деплой: FTP, Apache (.htaccess SPA fallback). Build: `vite build` + prerender 7 маршрутов → dist/{path}/index.html.
...
### 4.4 SEO этап 2 — prerender 7 страниц (head + JSON-LD в HTML)

```
Контекст: marviko-sunny-spaces_site. Этап 1 (schema.ts + PageSeo) сделан. Нужен статический <head> в исходном HTML для SEO-аудитов и Яндекса.

Цель: после `npm run build` в dist/ для каждого публичного маршрута свой index.html с правильными title, meta, canonical, og и JSON-LD из schema.ts (без дублирования вручную).

---

## 1. Маршруты

src/config/prerender.ts — PRERENDER_PATHS (7 URL):
/, /windows, /doors, /partitions, /windowsills, /accessories, /portfolio
...
После vite build:
- Прочитать dist/index.html как shell
- Для каждого маршрута: подставить SEO_BY_PATH + getSchemasForPath(route) в <head>
- Записать dist/index.html (/) и dist/{segment}/index.html

package.json: "build": "vite build && npm run prerender", "prerender": "tsx scripts/prerender.ts"

PageSeo/Helmet оставить — после гидрации поведение как сейчас.

---

## 3. Деплой

Весь dist/ на FTP. Apache отдаёт windows/index.html для /windows/ (редирект с /windows).
...
grep -c 'application/ld+json' dist/windows/index.html → 2
title в dist/windows/index.html — страница окон, не главная
```
...
Build включает prerender (scripts/prerender.ts). Не force push. Коммиты только по моей просьбе.
```
...
| SEO meta | 1.6, 1.7, 4.1, 4.2 |
| SEO prerender (head) | 4.4 |
