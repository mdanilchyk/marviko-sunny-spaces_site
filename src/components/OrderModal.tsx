          ...
          <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <motion.div className="relative bg-card rounded-xl p-6 sm:p-8 w-full max-w-md shadow-2xl border border-border" initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}>
              ...
              <div className="flex flex-col gap-4">
                ...
                <div>
                  <input type="tel" placeholder={SITE.phonePlaceholder} value={form.phone} onChange={(e) => { setForm({ ...form, phone: formatPhoneInput(e.target.value) }); setErrors({ ...errors, phone: false }); }} className={`w-full px-4 py-3 rounded-lg bg-background text-sm border focus:outline-none transition-colors ${errors.phone ? 'border-destructive' : 'border-border focus:border-primary'}`} disabled={sending} maxLength={17} inputMode="tel" />
                  {errors.phone && <p className="text-xs text-destructive mt-1">{getPhoneValidationError(form.phone) ?? "Пожалуйста, введите номер телефона"}</p>}
                </div>
